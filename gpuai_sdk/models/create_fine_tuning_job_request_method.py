...
class CreateFineTuningJobRequestMethod(BaseModel):
    """
    CreateFineTuningJobRequestMethod
    """ # noqa: E501
    type: StrictStr
    lora: Optional[CreateFineTuningJobRequestMethodLora] = None
    __properties: ClassVar[List[str]] = ["type", "lora"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['lora', 'qlora']):
            raise ValueError("must be one of enum values ('lora', 'qlora')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "lora": CreateFineTuningJobRequestMethodLora.from_dict(obj["lora"]) if obj.get("lora") is not None else None
        })
        return _obj
