...
class CommunityMachineDelistResult(BaseModel):
    """
    ...
    status: StrictStr
    __properties: ClassVar[List[str]] = ["machine_id", "status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['draining', 'delisted']):
            raise ValueError("must be one of enum values ('draining', 'delisted')")
        return value
